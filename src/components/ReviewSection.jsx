...
const ReviewSection = ({ vacationSku }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                setMsg({ type: 'success', text: 'Ulasan berjaya dihantar!' });
            ...
            setMsg({ type: 'error', text: 'Ralat menghantar ulasan.' });
        } finally {
    ...
    return (
        <div style={{ marginTop: '40px', borderTop: '1px solid #e2e8f0', paddingTop: '40px', fontFamily: "'Inter', sans-serif" }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                <div>
                    <h2 style={{ fontSize: '24px', fontWeight: '700', color: '#1e293b', margin: 0 }}>Ulasan Pelanggan</h2>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}>
                        <div style={{ display: 'flex' }}>{renderStars(Math.round(stats.avgRating))}</div>
                        <span style={{ fontSize: '14px', color: '#64748b', fontWeight: '600' }}>
                            {stats.avgRating.toFixed(1)} / 5 ({stats.count} ulasan)
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '40px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    {reviews.length === 0 ? (
                        <p style={{ color: '#94a3b8', fontStyle: 'italic' }}>Belum ada ulasan untuk pakej ini.</p>
                    ) : (
                        reviews.map(review => (
                ...
                <div style={{
                    ...
                    <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <MessageSquare size={18} color="#007bff" /> Tulis Ulasan
                    </h3>
                    <form onSubmit={handleSubmit}>
                        <div style={{ marginBottom: '16px' }}>
                            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>Nama Anda</label>
                            <input
                        ...
                        <div style={{ marginBottom: '16px' }}>
                            ...
                            >
                                ...
                                <option value="2">2 Bintang (Kurang Baik)</option>
                                <option value="1">1 Bintang (Lemah)</option>
                            </select>
                        </div>
                        <div style={{ marginBottom: '20px' }}>
                            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '6px' }}>Komen</label>
                            <textarea
